...
var userName = document.getElementById("currentUser").value;//当前登录用户的昵称
...
function deleteTopic(topicID) {
    if(confirm("确认删除话题吗？")){
        $.ajax({
            ...
            success: function (data) {
                if (data.success != null && data.success === true && data.data === true) {
                    alert("删除话题成功！")
                    window.location.href = "/user/" + userName;
                } else if (data.success != null && data.success === true && data.data === false) {
                    alert("该话题已被管理员屏蔽，暂时无法删除或编辑！")
                }else {
                    alert(data.error);
                }
            },
